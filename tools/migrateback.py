'''
17/02/2016
tool per migrare da django a pelican
'''
...
"""
post_list = Post.objects.all().prefetch_related('category').prefetch_related('tags')
...
for post in post_list:
    print(post.slug)
    with codecs.open(BASE_DIR +'\\pelicanPosts\\'+post.slug+'.md', 'w',  encoding='utf-8') as f:
        f.write('Title: ' + post.title)
        f.write('Date: ' + post.creation_date.strftime('%Y-%m-%d %H:%M')+ '\n')
        f.write('Modified: ' + post.modified_date.strftime('%Y-%m-%d %H:%M')+ '\n')
        f.write('Category: ' + post.category.title + '\n')
        tags=[]
